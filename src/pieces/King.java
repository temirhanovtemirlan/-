...
public class King extends Piece{
	...
	private int x,y; //Дополнительные переменные для класса Короля для отслеживания позиции короля
	...
	//Функция перемещения для короля, переопределенного с частей
	public ArrayList<Cell> move(Cell state[][],int x,int y)
	{
		// Король может двигаться только на один шаг. Таким образом, все смежные 8 ячеек были рассмотрены.
		possiblemoves.clear();
	...
	// Функция проверки угрозы короля
        // Он проверяет, есть ли какая-либо часть противоположного цвета, которая может атаковать короля для заданного состояния доски
	public boolean isindanger(Cell state[][])
    {
		
		//Проверка атаки слева, справа, вверх и вниз
    	for(int i=x+1;i<8;i++)
    	{
    	...
    	//проверка атаки с диагонального направления
		...
		//Проверка атаки с Коня противоположного цвета
		...
		//Проверка атаки с пешки противоположного цвета
